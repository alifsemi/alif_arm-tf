...
#define AXI_CLOCK 400000000 /* 400 MHz */
...
typedef struct _ospi_hyperram_xip_config {
    ...
    /**< Optional device specific initialization needed by the
      *  hyperram device  */
...
/**
  ...
  \brief      Initialize OSPI Hyerbus xip configuration. After a successful
              return from this function, the OSPI XIP region (for the OSPI
              instance specified in the ospi_hyerram_xip_config input parameter)
              will be active and can be used to directly read/write the memory
              area provided by the hyperram device.
